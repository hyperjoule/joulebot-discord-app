...
db.serialize(() => {
	...
	const personalityTitles = [	
		{ label: '😇 Playful', value: '0' },
		{ label: '😈 Mischevious', value: '1' },
		{ label: '🧠 Super Genius', value: '2' },
		{ label: '☠️ Apocalyptic Pessimist', value: '3' },
		{ label: '🤪 Nonsensical Haiku Bot', value: '4' },
		{ label: '🌞 \'Yes\' Bot', value: '5' },
		{ label: '🔮 Mystical Fortune Teller', value: '6' },
		{ label: '🤖 Companion', value: '7' }
	]

	const stmt = db.prepare('INSERT INTO personalities (id, label, content, temperature) VALUES (?, ?, ?, ?)')

	for (let i = 0; i < personalityArray.length; i++) {
		stmt.run(i, personalityTitles[i].label, personalityArray[i], temperatureArray[i])
	}
